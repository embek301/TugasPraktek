...
class ClockOut extends Controller
{
    ...
    public function index()
    {
        ...
        $isWithinAllowedTime = ($currentHour > 4);

        return view('content.Employee.izin.clockout', compact('pageTitle', 'users', 'dateInGMTPlus7', 'idClockOut', 'isWithinAllowedTime','isPenilai2'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $terlambat->alasan = $request->alasanClockOut;
            $penilai2Name = auth()->user()->penilai2;
            $terlambat->last = $penilai2Name;
            $terlambat->jenis = "Izin No Clock Out";
            ...
            if ($penilai2Mail) {
                ...
                Mail::to($penilai2Mail)->send(new noClockOutNotification($terlambat));

...
class LaporanSakit extends Controller
{
    /**
    ...
    {
        $pageTitle = 'Data Laporan Izin Sakit';
        $user = auth()->user();
        $userName = $user->who;
        ...
        $tanggalAkhir = '';
        $cabang = '';
        return view('content.Employee.data-laporan.izin-sakit', compact('pageTitle', 'isPenilai2', 'terlambatData', 'cabs', 'tanggalAwal', 'tanggalAkhir', 'cabang'));
    ...
    public function filterDataSakit(Request $request)
    {
        ...
        $terlambatData = Terlambat::join('users', 'users.who', '=', 'izin-terlambat.nama')
            ...
            ->where('izin-terlambat.jenis', 'izin sakit')
    ...
    public function exportExcelSakit(Request $request)
    {
        $pageTitle = 'Data Laporan izin Sakit ';
        $tanggalAwal = $request->input('tanggal_awal');
        $tanggalAkhir = $request->input('tanggal_akhir');
        ...
        return Excel::download(new SakitExport($judul, $terlambatData), $filename);
    }



    public function exportPDFSakit(Request $request)
    {
        $tanggalAwal = $request->input('tanggal_awal');
        $tanggalAkhir = $request->input('tanggal_akhir');
        $cabang = $request->input('cabang');
        ...
        $pdf = PDF::loadView('content.Employee.data-laporan.pdf.pdfSakit', compact('terlambatData', 'bulan', 'jenisLaporan'))->setPaper('a4');

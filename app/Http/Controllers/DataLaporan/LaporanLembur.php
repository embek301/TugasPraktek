...
class LaporanLembur extends Controller
{
    /**
    ...
    {
        $pageTitle = 'Data Laporan Izin Lembur';
        $user = auth()->user();
        $userName = $user->who;
        ...
        $tanggalAkhir = '';
        $cabang = '';
        return view('content.Employee.data-laporan.izin-lembur', compact('pageTitle', 'isPenilai2', 'terlambatData', 'cabs', 'tanggalAwal', 'tanggalAkhir', 'cabang'));
    ...
    public function filterDataLembur(Request $request)
    {
        ...
        $terlambatData = Terlambat::join('users', 'users.who', '=', 'izin-terlambat.nama')
            ...
            ->where('izin-terlambat.jenis', 'Izin Lembur')
    ...
    public function exportExcelLembur(Request $request)
    {
        $pageTitle = 'Data Laporan Izin Lembur ';
        $tanggalAwal = $request->input('tanggal_awal');
        $tanggalAkhir = $request->input('tanggal_akhir');
        ...
        return Excel::download(new LemburExport($judul, $terlambatData), $filename);
    }



    public function exportPDFLembur(Request $request)
    {
        $tanggalAwal = $request->input('tanggal_awal');
        $tanggalAkhir = $request->input('tanggal_akhir');
        $cabang = $request->input('cabang');
        ...
        $pdf = PDF::loadView('content.Employee.data-laporan.pdf.pdfLembur', compact('terlambatData', 'bulan', 'jenisLaporan'))->setPaper('a4');

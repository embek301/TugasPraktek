...
use App\Exports\NoClockOutExport;
use Illuminate\Support\Facades\DB;
use Maatwebsite\Excel\Facades\Excel;
use PDF;
use Illuminate\Support\Facades\Log;

class LaporanClockOut extends Controller
{
    /**
    ...
    {
        $pageTitle = 'Data Laporan Izin No Clock Out';
        $user = auth()->user();
        $userName = $user->who;
        ...
        $tanggalAkhir = '';
        $cabang = '';
        return view('content.Employee.data-laporan.izin-no-clock-out', compact('pageTitle', 'isPenilai2', 'terlambatData', 'cabs', 'tanggalAwal', 'tanggalAkhir', 'cabang'));
    ...
    public function filterDataClockOut(Request $request)
    {
        ...
        $terlambatData = Terlambat::join('users', 'users.who', '=', 'izin-terlambat.nama')
            ...
            ->where('izin-terlambat.jenis', 'Izin No Clock Out')
    ...
    public function exportExcelClockOut(Request $request)
    {
        $pageTitle = 'Data Laporan Izin No Clock Out ';
        $tanggalAwal = $request->input('tanggal_awal');
        $tanggalAkhir = $request->input('tanggal_akhir');
        ...
        return Excel::download(new NoClockOutExport($judul, $terlambatData), $filename);
    }



    public function exportPDFClockOut(Request $request)
    {
        $tanggalAwal = $request->input('tanggal_awal');
        $tanggalAkhir = $request->input('tanggal_akhir');
        $cabang = $request->input('cabang');
        ...
        $pdf = PDF::loadView('content.Employee.data-laporan.pdf.pdfNoClockOut', compact('terlambatData', 'bulan', 'jenisLaporan'))->setPaper('a4');
